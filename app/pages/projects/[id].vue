<template>
  <div>
    ...
    <div v-if="project" style="margin-top:1rem">
      <div style="display:flex; gap:.5rem; align-items:center; justify-content:space-between;">
        <div>
          ...
          <div class="small" style="margin-top:.4rem">{{ project.description }}</div>
        </div>
        ...
        <div style="display:flex; gap:.75rem; align-items:center;">
          ...
          <div>
            <label for="status-select" class="text-xs muted" style="display:block;margin-bottom:.25rem;">Completion</label>
            <select
              id="status-select"
              class="input"
              :value="statusSelect"
              ...
              :disabled="busy"
              aria-label="Change project completion status"
      ...
      <div style="margin-top:1rem;">
        ...
        <div style="margin-top:1rem">
          <h3 style="margin-bottom:.5rem">Notes</h3>
          <NotesList :notes="project.notes" />
          <form @submit.prevent="addNote" class="form" style="margin-top:.75rem">
            <textarea v-model="note" rows="3" placeholder="Add a new note (Markdown supported)" class="input" required></textarea>
    ...
    <div v-else class="muted" style="margin-top:1rem">Project not found.</div>
  </div>
</template>
...
async function confirmDelete() {
  ...
  const ok = confirm(`Delete project "${project.value.title}"? This cannot be undone.`);
  if (!ok) return;
...
const statusClass = computed(() => {
  switch (project.value?.status) {
    ...
    case 'in-progress': return 'badge badge--warn';
    case 'not-started': return 'badge badge--muted';
...
.text-xs { font-size: .78rem; color: #6b7280; }
</style>